        ...
        <label>
          ...
          <Box mt='1%' display={'flex'} justifyContent={'space-between'}>
            {LocationArr.map((ele, index) =>
              ele === "51-200" ? <Text key={index} p={'2px 10px'} borderRadius={'1px'} bg='blue.500' color={'white'}>{ele}</Text> :
                <Text key={index} p={'2px 10px'} borderRadius={'1px'} bg='blackAlpha.100'>{ele}</Text>
            )}
          </Box>
        </label>
        <br />
